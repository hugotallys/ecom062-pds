...
# ╔═╡ 5a5be5d7-f25c-49e5-86cb-ec1fc33800b2
html"
<center>
	<h4>Universidade Federal de Alagoas</h4>
...
<p><b>Disciplina</b>: Processamento Digital de Sinais</p>

<p><b>Professor</b>: Thiago D. Cordeiro</p>

<p><b>Aluno</b>: Hugo Tallys Martins Oliveira</p>

<hr>

<center><h4>Atividade 01 - AB2</h4></center>

<hr>
"

# ╔═╡ bd63c70a-a0ee-4b70-98d0-486902e0920a
md"Importando os pacotes necessários:"
...
$$0.89125 \leq |H(e^{j\omega})| \leq 1 \ , \ 0 \leq \omega \leq 0.2\pi$$
$$|H(e^{j\omega})| \leq 0.17783 \ , \ 0.3\pi \leq \omega \leq \pi$$
...
$$0.89125 \leq |H_{lp}(e^{j\theta})| \leq 1 \ , \ 0 \leq \theta \leq 0.2\pi$$
$$|H_{lp}(e^{j\theta})| \leq 0.17783 \ , \ 0.3\pi \leq \theta \leq \pi$$

Utilizando a técnica de transformação de frequências para filtros do tipo passa-baixa,
transforme o filtro descrito em um filtro passa-alta com frequência de corte $\omega_p = 0.6π$. Apresente os diagramas de magnitude para os dois filtros e aplique sinais na entrada de ambos para comprovar que as funções de transferência possuem desempenho adequado.
"
...
# ╔═╡ 0d2e1142-28af-43ca-ac9f-fe1fd9d2d748
md"Instânciando o filtro passa baixa de ordem 4, com o desvio 0.56 dB e frequência de corte desejada 0.2π (normalizada):"

# ╔═╡ fe3904df-5f7f-4ca6-b6cb-1f512c568a85
Hlp = signal.lti(signal.cheby1(4, 0.56, 0.2)...);

# ╔═╡ 0fd50525-2162-45ee-be8d-539d5b9f518f
md"Para criar o filtro passa-alta, basta especificar a nova frequência de corte e tipo `\"highpass\"`:"

# ╔═╡ 3d72d4f4-5a95-4cff-920d-1e041d749ed7
Hhp = signal.lti(signal.cheby1(4, 0.56, 0.6, "highpass")...);

# ╔═╡ 46cf0ba5-764f-4867-b13c-5f42391a7fea
md"Plotando os diagramas de magintude:"

# ╔═╡ 6e50ec5a-7524-4c6a-acd2-e3c3fa9e2145
begin
...
# ╔═╡ 24ae702e-c611-48be-91c2-e1959afc78b4
md"Aplicando o filtro passa-baixa ao sinal de entrada, obtemos um resultado similar ao anterior:"

# ╔═╡ a06a36ad-27da-43f8-848a-679db620aebd
begin	
...
# ╔═╡ ee887b07-b610-4bcf-9671-de73bf63f81c
md"Aplicando o filtro passa-alta no sinal de entrada, obtemos a filtragem apenas do sinal base, deixando passar o ruído."

# ╔═╡ 89c86cd1-3aba-4643-8c3a-f04b8379b6b7
begin	
...
# ╔═╡ 263f0de5-9df6-42f3-8c2d-7f7b68a2c79f
md"
**Problema 3**

Com o uso das fórmulas utilizadas no projeto de filtros pelo método da janela de
*Kaiser*, projete um filtro passa-baixa FIR para atender as seguintes especificações de desempenho:
...
# ╔═╡ 0b5891c1-d886-4d79-b729-12a3378695ec
md"
**Solução**

De acordo com as especificações dadas, para o projeto por janela, o filtro resultante terá o mesmo erro de pico $\delta$ na faixa de passagem e na faixa de rejeição. Como os filtros projetados pelo método de janelamento têm inerentemente $\delta_1 = \delta_2$, precisamos definir $\delta = 0.001$. Em seguida, encontramos a frequência de corte do filtro passa-baixas, definindo que:

$$\omega_c = \frac{\omega_p + \omega_s}{2} = 0.5\pi$$

devido à simetria da aproximação na descontinuidade de $H_d(e^{j\omega})$.

Para determinar os parâmetros da *janela de Kaiser*, primeiro calculamos $\omega = \omega_s - \omega_p = 0.2\pi$ e $A = −20 log_{10}\delta = 60$, a largura da região de transição e o desvio de magintude desejado em dB respectivamente.

Substituímos essas duas quantidades nas equações 7.75 e 7.76 **[1]** para obter os valores requeridos de $\beta$ e $M$, calculados empiricamente o que nos dá:

$$β = 5.653 \ , \ M = 37$$

Por fim, calculamos a resposta ao impulso do filtro utilizando as equações 7.71 e 7.72 **[1]**, obtendo:

$$h[n] = \frac{sen(\omega_c(n-\alpha))}{\pi(n-\alpha)} \frac{I_0[\beta(1-[(n-\alpha)/\alpha]^2)^{1/2}]}{I_0(\beta)} \ , \ 0 \leq m \leq M$$

com $h[n] = 0$ caso contrário.
"

# ╔═╡ fefbd2ca-ea52-4064-8b1e-f54ac7d21937
begin
	ω_p, ω_s = 0.4π, 0.6π
	δ = 0.001
...
# ╔═╡ 094cd041-4a56-430c-93a5-5ff010532f35
begin
	ω_c = (ω_s + ω_p)/2π
	kaiser = signal.firwin(M, ω_c)
end;

# ╔═╡ 96d68966-2081-4b76-960d-e5ac91909d44
md"Plotando a curva da *janela de Kaiser* encontrada:"

# ╔═╡ 168bdfd5-b098-4aec-a22d-40571a032889
plot(
...
# ╔═╡ c6e860b2-67c1-408d-80aa-94590acccc81
md"Plotando o diagrama de magintude:"

# ╔═╡ a67b1408-5cd9-4310-b73f-95b1b26f58f0
begin
...
# ╔═╡ 3fbd8e39-658c-41d4-ba29-95dfba555ba8
md"
### Referências
"

# ╔═╡ 46bc7b5d-581f-48ce-bace-a102b2abf0ba
md"
...
**[2]** Documentação SciPy-Signal (https://docs.scipy.org/doc/scipy/reference/signal.html?highlight=signal#module-scipy.signal)
"
